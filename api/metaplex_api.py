...
class MetaplexAPI():
    ...
    def deploy(self, api_endpoint, name, symbol, fee, max_retries=3, skip_confirmation=False, max_timeout=60, target=20, finalized=True):
        """
        ...
        """
        try:
            tx, signers, contract = deploy(api_endpoint, self.account, name, symbol, fee)
            print(contract)
            resp = execute(
                api_endpoint,
                tx,
    ...
    def update_token_metadata(self, api_endpoint, mint_token_id, link,  data, fee, creators_addresses, creators_verified, creators_share, max_retries=3, skip_confirmation=False, max_timeout=60, target=20, finalized=True, supply=1 ):
            """
